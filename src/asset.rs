...
use cw_storage_plus::{Bound, Key, KeyDeserialize, Prefixer, PrimaryKey};
...
impl<'a> Into<Bound<'a, Asset>> for Asset {
    fn into(self) -> Bound<'a, Asset> { Bound::exclusive(Into::<Vec<u8>>::into(self)) }
}
...
impl KeyDeserialize for Asset {
    ...
    fn from_vec(mut value: Vec<u8>) -> StdResult<Self::Output> {
        match value.pop().unwrap() {
            0 => Ok(Asset::Token {
                addr: Addr::from_vec(value)?,
            }),
            1 => Ok(Asset::NativeToken {
                denom: String::from_vec(value)?,
            }),

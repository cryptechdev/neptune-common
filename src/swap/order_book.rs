...
use cosmwasm_std::{CosmosMsg, Deps, Uint256, QueryRequest, Decimal256, Env};
use injective_cosmwasm::{InjectiveQuery, MarketId, InjectiveRoute, MarketMidPriceAndTOBResponse, SpotMarketResponse, OrderSide, InjectiveMsg, SpotOrder, OrderInfo, OrderType, SpotMarket, get_default_subaccount_id_for_checked_address, exchange::response::QueryOrderbookResponse, QueryMarketAtomicExecutionFeeMultiplierResponse};
use injective_math::FPDecimal;
use crate::{asset::AssetInfo, error::{NeptuneResult, NeptuneError}, query_wrapper::QueryWrapper, msg_wrapper::MsgWrapper, injective::{into_fp_decimal, into_uint_256, into_decimal_256}};
...
impl Swap for OrderBook {
    fn swap(
        &self, deps: Deps<QueryWrapper>, env: &Env, offer_asset: &AssetInfo, _ask_asset: &AssetInfo, offer_amount: Uint256,
    ) -> NeptuneResult<Vec<CosmosMsg<MsgWrapper>>> {
        if let Some(msg) = market_order_offer(deps, env, self.market_id.clone(), offer_asset, offer_amount)? {
            Ok(vec![msg])
        } else {
            Ok(vec![])
        }
    ...
    fn swap_ask(
        &self, deps: Deps<QueryWrapper>, env: &Env, _offer_asset: &AssetInfo, ask_asset: &AssetInfo, ask_amount: Uint256,
    ) -> NeptuneResult<Vec<CosmosMsg<MsgWrapper>>> {
        if let Some(msg) = market_order_ask(deps, env, self.market_id.clone(), ask_asset, ask_amount)? {
            Ok(vec![msg])
        } else {
            Ok(vec![])
        }
    ...
    fn query_sim(
        ...
        let AssetInfo::NativeToken { denom: offer_denom } = offer_asset else {
            return Err(NeptuneError::Generic("Only native tokens are supported".to_string()));
        };
    ...
    fn query_swap_ratio(
        ...
        let ask_amount = if offer_denom == &spot_market.quote_denom {
            let buy_quantity = get_buy_quantity(&spot_market, fee_rate, &order_book, offer_amount)?.max(spot_market.min_quantity_tick_size);
            offer_amount = get_buy_offer_amount(&spot_market, fee_rate, &order_book, buy_quantity)?;
            buy_quantity
            
        } else if offer_denom == &spot_market.base_denom {
            offer_amount = tick_round_down(offer_amount, spot_market.min_quantity_tick_size).max(spot_market.min_quantity_tick_size);
        ...
        if ask_amount.is_zero() {
            return Err(NeptuneError::Generic("Market is empty, could not calculate swap ratio".to_string())); 
        }
...
pub fn market_order_offer(deps: Deps<QueryWrapper>, env: &Env, market_id: MarketId, offer_asset: &AssetInfo, offer_amount: Uint256) -> NeptuneResult<Option<CosmosMsg<MsgWrapper>>> {
...
pub fn market_order_ask(deps: Deps<QueryWrapper>, env: &Env, market_id: MarketId, ask_asset: &AssetInfo, ask_amount: Uint256) -> NeptuneResult<Option<CosmosMsg<MsgWrapper>>> {
    ...
    if &spot_market.base_denom == ask_denom {
        let expected_offer_amount = get_buy_offer_amount(&spot_market, fee_rate, &order_book, ask_amount)?;
        buy(env, &spot_market, fee_rate, expected_offer_amount, ask_amount)
...
fn get_buy_quantity(
    ...
    for sell_order in &order_book.sells_price_level {
        ...
        if remaining_offer_amount > sell_order_base_amount {
            ...
            quantity += (remaining_offer_amount / ((FPDecimal::one() + fee_rate) * sell_order_price)).int();
            break;
        }
    }
...
fn get_sell_quantity(
    ...
    for buy_order in &order_book.buys_price_level {
        ...
        if remaining_ask_amount > buy_order_base_amount {
            ...
            quantity += (remaining_ask_amount / ((FPDecimal::one() + fee_rate) * buy_order_price)).int();
...
fn get_sell_ask_amount(
    ...
    for buy_order in &order_book.buys_price_level {
        ...
        if buy_order_quantity > remaining_quantity {
            ask_amount += ((remaining_quantity * buy_order_price) / (FPDecimal::one() + fee_rate)).int();
            break;
        } else {
            ask_amount += ((buy_order_quantity * buy_order_price) / (FPDecimal::one() + fee_rate)).int();
...
fn buy(
    ...
    let quantity = tick_round_up(quantity, spot_market.min_quantity_tick_size);
    if quantity.is_zero() { return Ok(None) }
    if expected_offer_amount.is_zero() { return Ok(None) }
    
    let expected_offer_amount_less_fees = (expected_offer_amount / (FPDecimal::one() + fee_rate)).int();
    ...
    let wrapper = MsgWrapper {
        ...
        msg_data: InjectiveMsg::CreateSpotMarketOrder { sender: env.contract.address.clone(), order },
    };
...
pub fn query_spot_market_mid_price_and_tob(
    deps: Deps<QueryWrapper>, market_id: MarketId
...
fn query_spot_market(
    ...
    let spot_market = res.market.ok_or(NeptuneError::Generic("Spot market not found".to_string()))?;
...
fn query_atomic_fee_execution_multiplier(
    ...
    let res: QueryMarketAtomicExecutionFeeMultiplierResponse = deps.querier.query(&query_request)?;
...
fn query_total_fees(
    ...
    let multiplier = query_atomic_fee_execution_multiplier(deps, spot_market.market_id.clone()).unwrap();

...
use cw_storage_plus::{Path, Map, PrimaryKey};
...
where
    V: Serialize, for<'de> V: Deserialize<'de>,
    (K2, V): Serialize,
    ...
    K2: PrimaryKey<'a>,
{
    pub const fn new(namespace1: &'a str, namespace2: &'a str) -> Self {
        Self {
            main_map: Map::new(namespace1),
            key_map: Map::new(namespace2)
        }
    }

    pub fn key1(&self, key1: K1) -> Path<(K2,V)> {
        self.main_map.key(key1)
    }
    ...
    pub fn load1(&self, store: &dyn Storage, key1: K1) -> StdResult<V> {
        Ok(self.main_map.load(store, key1)?.1)
    }
